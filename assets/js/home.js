(function () {
    ...
    // play background videos when user window's y offset is correct
    window.addEventListener('scroll', function(event){
        var currentYOffset = this.scrollY;
        if (currentYOffset > 550) {
            myVideo2.play()
        }

        if (currentYOffset > 1250) {
            myVideo3.play()
        }

        if (currentYOffset > 2050) {
            myVideo4.play()
        }
    })
    ...
    var getSuperHeroCard = function (sourceImg, name, id) {
        ...
        imgV.setAttribute("onerror", "this.onerror= null; this.src='assets/images/imgErr.jpg';");
    ...
    textBar.onkeyup = async function () {
        ...
        let response = await fetch(`https://www.superheroapi.com/api.php/717213773065696/search/${name}`).catch(e => {
            console.log("error");
        });
    ...
    //     let response = await fetch(`https://www.superheroapi.com/api/717213773065696/${id}`, { signal }).catch(e => {
    //         console.log("error");
    //     });
    //     let data = await response.json();
    //     renderOnLoad(data);
